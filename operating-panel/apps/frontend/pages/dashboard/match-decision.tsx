...
export default function MatchDecision() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/decision/match', {
        method: 'POST',
      ...
      if (response.ok) {
        alert('Match decision applied successfully!');
        router.push('/dashboard');
      } else {
        const data = await response.json();
        setError(data.message || 'Failed to apply match decision');
      }
    } catch (error) {
      console.error('Error applying match decision:', error);
